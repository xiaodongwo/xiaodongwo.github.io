...
($q(x_t \mid x_{t-1})$ is a simple Gaussian.)  
We need to make some assumptions to make things easier to compute.  
Assume the diffusion process varies at a slow flowing speed ($\beta_t \ll 1$),  
we could approximate the learned $p_\theta(x_{t-1}\mid x_t)$ as a Gaussian distribution.  
Note that the notation of PDF switches from $q(x)$ to $p(x)$ to distinguish the forward and backward process.
